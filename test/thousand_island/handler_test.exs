defmodule ThousandIsland.HandlerTest do
  ...
  describe "telemetry" do
    ...
    test "it should send `handshake` telemetry event once socket handshake completes" do
      {:ok, collector_pid} =
        start_supervised(
          {ThousandIsland.TelemetryCollector, [[:thousand_island, :connection, :handshake]]}
        )
      ...
      assert ThousandIsland.TelemetryCollector.get_events(collector_pid)
             ~> [
               {[:thousand_island, :connection, :handshake], %{time: integer()},
                %{span_id: string()}}
             ]
    end
